...
Options:
    ...
    --1rtt-pk PK          Read known server certificate to activate 1RTT-KEMTLS
...
fn load_pk_and_epoch(pkfile: &str, epochfile: &str) -> 
            (Vec<u8>,rustls::epoch::Epoch){
    let epoch = load_epoch(epochfile);
    let pk = load_pk(pkfile);
    (pk,epoch)
}
