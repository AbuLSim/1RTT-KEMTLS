...
pub trait KeyScheduleComputesServerFinish {
    fn sign_server_finish(&self, hs_hash: &[u8]) -> Vec<u8>;
}
...
impl KeyScheduleEarly {
    ...
    pub fn resumption_psk_binder_key_and_sign_verify_data(&self, hs_hash: &[u8]) -> Vec<u8> {
        ...
        self.ks
            .sign_verify_data(&resumption_psk_binder_key, hs_hash, None)
    }
...
impl KeyScheduleHandshake {
    pub fn authenticate_handshake(&mut self, server_ciphertext: &[u8]) {
        debug_assert!(self.authenticated == false, "can't do this twice!");
        self.ks.input_secret(server_ciphertext);
        self.authenticated = true;
    }

    pub fn assert_authenticated(&self) {
    ...
    pub fn client_authenticated_handshake_traffic_secret(
        &mut self,
        hs_hash: &[u8],
        key_log: &dyn KeyLog,
        client_random: &[u8; 32],
    ) -> hkdf::Prk {
        debug_assert!(self.authenticated, "haven't received a shared secret");
        let secret = self.ks.derive_logged_secret(
    ...
    pub fn server_authenticated_handshake_traffic_secret(
        &mut self,
        hs_hash: &[u8],
        key_log: &dyn KeyLog,
        client_random: &[u8; 32],
    ...
    ) -> KeyScheduleTrafficWithServerFinishedPending {
        if let Some(ss) = client_shared_secret {
            self.ks.input_secret(ss);
        } else {
            self.ks.input_empty();
...
/// KeySchedule during traffic stage, retaining the ability to calculate the server's
/// finished verify_data, and incrementally generate the first traffic keys.
/// For KEMTLS
pub struct KeyScheduleTrafficWithServerFinishedPending {
    ks: KeySchedule,
    current_client_traffic_secret: Option<hkdf::Prk>,
    current_server_traffic_secret: Option<hkdf::Prk>,
...
impl KeyScheduleComputesClientFinish for KeyScheduleTrafficWithServerFinishedPending {
    fn sign_client_finish(&self, hs_hash: &[u8]) -> Vec<u8> {
        self.ks.sign_finish_kemtls(&self.ks.current, hs_hash, false)
    }
}

impl KeyScheduleComputesServerFinish for KeyScheduleTrafficWithServerFinishedPending {
    fn sign_server_finish(&self, hs_hash: &[u8]) -> Vec<u8> {
...
impl KeyScheduleTrafficWithServerFinishedPending {
...
impl KeyScheduleComputesClientFinish for KeyScheduleTrafficWithClientFinishedPending {
    fn sign_client_finish(&self, hs_hash: &[u8]) -> Vec<u8> {
        self.ks
...
impl KeySchedule {
    ...
    fn sign_finish(&self, base_key: &hkdf::Prk, hs_hash: &[u8]) -> Vec<u8> {
        self.sign_verify_data(base_key, hs_hash, None)
    }

    /// Sign the finished message for KEMTLS
    fn sign_finish_kemtls(&self, base_key: &hkdf::Prk, hs_hash: &[u8], is_server: bool) -> Vec<u8> {
        self.sign_verify_data(
            base_key,
            hs_hash,
            Some(if is_server { b"server" } else { b"client" }),
        )
    }
    ...
    fn sign_verify_data(
        ...
        let label = label.unwrap_or(b"finished");
        let hmac_alg = self.algorithm.hmac_algorithm();
        let hmac_key = hkdf_expand(base_key, hmac_alg, label, &[]);
